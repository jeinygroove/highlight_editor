...
Данный Си-код, по-сути, также выполняет операцию сопоставления шаблонов реальным аргументам функции. Но только в данном случае, программист должен сам написать код реализующий все это, да еще и убедиться, что этот он работает корректно.
...
И, допустим, мы мы хотим подсчитать общую стоимость всех элементов этого списка. Мы можем сделать это, например, вот так:
...
в различных контекстах: в объявлении функций, а так же в блоках case, if,
try..catch и в выражениях receive.

Всякий раз когда мы имеем набор шаблонов, с последующими за ними выражениями,
...
Перед тем как закончить этот раздел, давайте более детально рассмотрим функцию total. Функция total(L) работает в зависимости от результата анализа аргумента L. Тут возможно два варинта: или L - это не пустой список, или же наоборот. Мы реализовали клаузы для каждого возможного варианта, вот так:
...
funs ("фаны") это “анонимные” функции. Они называются так, потому что у них нет имени. Давайте немного поэксперементируем. Для начала объявим функцию и сопоставим ее с переменной Z:
...
Здесь X rem 2 вычесляет остаток от деления Х на 2, а оператор =:= сравнивает его с нулем. Теперь мы можем использовать функцию Even как аргумент функция map и filter:
...
Примечание: Данное определение функции map/2 скопировано нами из модуля lists стандартной библиотеки в модуль mylists. С модулем mylists вы можете делать все, что угодно. Но не при каких обстоятельствах, не пытайтесь создать свой собственный модуль с именем lists, если только вы абсолютно не уверены в том, что вы делаете.
...
 Когда я пишу программу, то я пользуюсь подходом: "напиши чуть-чуть - потом протестируй это". Я начинаю с маленького модуля, состоящего всего из нескольких функций, потом я компилирую его и тестирую с помощью нескольких команд в оболочка Эрланга. Когда он меня вполне удовлетворяет, я дописываю в нем еще несколько функций, компилирую его и тестирую. И так далее.

Часто я не представляю себе заране, какие именно структуры данных мне потребуются в моей программе, но по мере того, как я попробую несколько простых примеров, я уже могу оценить, какие подходящие варианты из них мне следует выбрать.
...
Отметим дополнительно, что набор (тьюпл) {Name, Number}  с права от знака "||" - это образец (паттерн), которому сопоставляется каждый элемент из списка Buy. А набор (тьюпл) слева от знака "||" - {Name, 2*Number} - называется конструктор.
...
Теперь мы разделяем список Т на два списка : один из элементов, которые меньше чем Pivot, а второй - из элементов, которые больше или равны Pivot:
...
Эта запись означает, что "когда Х это целое число и Х больше чем У и У меньше чем 6, то ...". Запятая, разделяющая отдельные проверки в контролере означает логическое "И".

is_tuple(T), size(T) =:= 6, abs(element(3, T)) > 5
element(4, X) =:= hd(L)
...

Первый контролер проверяет, что Х это либо cat (кошка), либо dog (собака). Второй контролер проверяет, что, либо Х - это целое число, которое больше чем У; либо, что абсолютное значение У меньше чем 23.
...
Данный узел Эрланга.
node(X)
Узел на котором Х был создан, где Х это может быть процесс, идентификатор, ссылка или порт.
round(X)
...
В строке 4 мы скопировали существующую запись. Синтаксис X1#todo{status=done} означает: создать копию записи Х1 (которая должна быть типа todo) и изменить в ней значение поля status на done (сделано). Обратите внимание, что это будет только копия. Исходная запись при этом не изменится.
...
До сих пор, мы использовали для решения всех своих задач только механизм сопоставления по образцу. Это делает Эрланг компактным и последовательным. Но, иногда, определять различные клаузы в функциях для каждого случая бывает довольно неудобно. В этом случе мы можем воспользоваться выражениями case и if.
...
Она вычисляется следующим образом: Сначала вычисляется контролер Guard1. Если его значение равно true, то тогда значение всего выражения if получается вычислением последовательности выражений Expr_seq1. Если же контролер Guard1 не сработал, то вычисляется Guard2 и так далее, пока кто-то из них на примет значение Истина (true). Если же такового не найдется, то будет поднято исключение.

Часто последним контролером выражения if бывает атом true, который "пропускает всех", тем самым гарантируя, что, по крайней мере одна из последовательностей выражений будет вычеслена, даже если все остальные охранники не сработают.
...
Когда исходный список закончится, сработае финальная клауза данной функции и итоговая переменная Result будет возвращена из данной функции.
...
Онако, следующая глава данной книги посвящена краткому рассмотрению темы работы с ошибками в Эрланге. После нее мы снова вернемся к последовательному программированию и рассмотрим в нем все, пока оставленные нами, детали.
